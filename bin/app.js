...
function tryLoadBaked() {
  // baked.js is generated by `scripts/bake.js` and embedded into the .exe.
  // If it's absent (dev builds) we fall back to config.json next to the exe.
  try {
    return require("./baked.js");
  } catch (_) {
    return null;
  }
}

function applyBaked(baked) {
  const map = {
    UNICOMMERCE_TENANT_URL: baked.unicommerceTenantUrl,
    UNICOMMERCE_USERNAME: baked.unicommerceUsername,
    UNICOMMERCE_API_KEY: baked.unicommerceApiKey,
    GOOGLE_SHEET_ID: baked.googleSheetId,
    DRR_WINDOW_DAYS: baked.drrWindowDays,
    REORDER_THRESHOLD_DAYS: baked.reorderThresholdDays,
    LEAD_TIME_DAYS: baked.leadTimeDays,
    SAFETY_STOCK_DAYS: baked.safetyStockDays,
    GET_SALE_ORDER_CONCURRENCY: baked.getSaleOrderConcurrency,
  };
  for (const [k, v] of Object.entries(map)) {
    if (v !== undefined && v !== null && v !== "") process.env[k] = String(v);
  }
  if (baked.googleServiceAccountJson) {
    process.env.GOOGLE_SERVICE_ACCOUNT_JSON =
      typeof baked.googleServiceAccountJson === "string"
        ? baked.googleServiceAccountJson
        : JSON.stringify(baked.googleServiceAccountJson);
  }
}
